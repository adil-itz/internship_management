 ...
 * Send email utility
 * Supports:
 * 1. Brevo HTTP API (BREVO_API_KEY) - Recommended for Render (Allows ANY recipient email for free)
 * 2. Resend HTTP API (RESEND_API_KEY) - Allowed for testing or domain owners
 * 3. Nodemailer SMTP (Gmail / Custom host) - For local dev
 */
export const sendEmail = async ({ to, subject, html, text, attachments }) => {
  try {
    ...
    console.log(`[EMAIL_DEBUG] ENV RESEND_API_KEY: ${resendKey ? 'DETECTED' : 'NOT FOUND'}`);
    console.log(`[EMAIL_DEBUG] --------------------------------------------------`);
    ...
    // ----------------------------------------------------
    console.log(`⚠️ [EMAIL] Neither BREVO_API_KEY nor RESEND_API_KEY found! Falling back to Nodemailer SMTP...`);

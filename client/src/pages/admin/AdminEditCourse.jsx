...
export default function AdminEditCourse({ darkMode, setDarkMode, user }) {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      ...
      try {
        ...
        console.error('Failed to load course for admin edit:', err);
  ...
  return (
    <DashboardLayout user={user} darkMode={darkMode} setDarkMode={setDarkMode}>
      {loading ? (
        ...
        <CourseForm initialData={initialData} isEditing={true} onSubmit={handleEditSubmit} backPath="/admin/courses" />
      )}
    </DashboardLayout>
  );
}

...
export default function AdminEditResource({ darkMode, setDarkMode, user }) {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchResource = async () => {
      ...
      try {
        ...
        console.error('Failed to load resource for admin edit:', err);
  ...
  return (
    <DashboardLayout user={user} darkMode={darkMode} setDarkMode={setDarkMode}>
      {loading ? (
        ...
        <ResourceForm initialData={initialData} isEditing={true} onSubmit={handleEditSubmit} backPath="/admin/resources" />
      )}
    </DashboardLayout>
  );
}
